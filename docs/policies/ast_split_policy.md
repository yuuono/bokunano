...
24操作から同じ操作を繰り返さない1〜3操作の順列を全列挙した時点では、各操作は1操作で1件、2操作で46件、3操作で1,518件、合計1,565意味ASTへ同数ずつ含まれていた。train内の件数差は、組合せ汎化用ASTの先行除外と、その後のハッシュ分割によって生じる。

組合せ汎化の5ペアに使う10操作は、各158意味ASTが`compositional`へ移る。その他14操作も、5ペアの第三操作として現れる各30意味ASTが`compositional`へ移る。
...
残った意味ASTは、操作ごとの含有件数を揃える層化抽出ではなく、1操作、2操作、3操作の各集合を正規化意味ASTのSHA-256順に並べ、総件数だけを固定して分割する。このため、同じ操作区分内にも数十件程度の差が生じる。分割は決定的で再現可能だが、操作別の周辺分布を均等化するものではない。

この差は、組合せ汎化テストで指定ペアをtrainから隔離する方針と、ハッシュ分割方式から生じたものであり、日本語指示生成や教師言い換えによって生じたものではない。また、1意味AST当たりの日本語指示数と、各操作を含む意味AST数は別の集計軸である。
