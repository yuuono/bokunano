...
日本語指示は2026年9月24日に、訓練用表現辞書だけを使って各意味AST最大20件生成した。反復汎化では日本語表現を新しい評価軸にせず、同じ操作を隣接して適用する意味構造だけを評価する。生成件数は2操作480件、3操作22,560件、合計23,040件である。詳細は[`rule_generated_instruction_results.md`](../results/rule_generated_instruction_results.md)に記録する。
