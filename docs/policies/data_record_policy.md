...
置換反映済み訓練指示と検証済み訓練コードは、次の順で結合する。

1. `spec_id`が一致することを確認する。
2. `semantic_ast`をキー順・空白なしのJSONへ正規化し、内容が一致することを確認する。
3. 指示とコードをそれぞれ元JSONLの順序で一対一対応させる。
4. 指示数が20件未満の意味ASTだけは、`code_style`別の採用数ができるだけ均等になるようコードを決定的に選ぶ。同数候補の順位は`pairing_seed`、`spec_id`、`code_style`、`code_id`のSHA-256で固定する。
5. 余ったコードは削除せず、元コード候補の全項目に`reason=instruction_shortfall`、件数、選抜方式、`pairing_seed`を加えた不採用JSONLへ保存する。

最終訓練レコードは全指示をちょうど一度、採用コードを高々一度使用する。採用コードと不採用コードは重複させず、その合計を入力コード候補総数と一致させる。

コード候補生成時に使った境界9件と`random_seed=0`のランダム32件は、共有テスト集合`build-verification-v1-seed-0-boundary-9-random-32`として別JSONへ保存する。最終レコードの`tests`には実入力を複製せず、このIDだけを格納する。これはbuild時の内部検証入力であり、学習プロンプトへ含めない。
